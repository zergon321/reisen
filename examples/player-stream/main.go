...
func (game *Game) Start(fname string) error {
	...
	handlers := &reisen.AVIOHandlers{}
	handlers.ReadSeeker = &readseek
	handlers.ReadPacket = func() ([]byte, int) {

		contents := make([]byte, reisen.IO_BUFFER_SIZE)
		len, err := readseek.Read(contents)
		if err != nil {
			empty := make([]byte, 0)
	...
	handlers.Seek = func(offset int64, whence int) int64 {
		if whence == 65536 { // avseek
			return -1
		}
		rs, err := readseek.Seek(offset, whence)
		if err != nil {
			fmt.Println(err)
			return 1
		}
	...
	// Open the media file.
	media, err := reisen.NewMediaFromReader("demo", handlers)
